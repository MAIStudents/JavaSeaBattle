...
public class Server {
    ...
    private Map<PointsImpl, Boolean> firstPlayerField;
    private Map<PointsImpl, Boolean> secondPlayerField;
    ...
    private void game() {
        ...
        while (playersOnline()) {
            ...
            if (response.equals(MISS)) {
                ...
                turnClient.sendMessage(WINNER);
                switchClient(turnClient).sendMessage(OPPONENT_IS_THINKING + " " + turn);
                switchClient(turnClient).sendMessage(LOSER);
    ...
    private void initializeField(String message, ClientHandler client) {
        ...
        Map<PointsImpl, Boolean> field = new HashMap<>();
        ...
        for (int i = 0; i < positions.length; ++i) {
            ...
            field.put(new PointsImpl(x0, y0), true);

            if (x0 == x1) {
                for (int j = y0 + 1; j <= y1; ++j) {
                    field.put(new PointsImpl(x0, j), true);
                }
            } else {
                for (int j = x0 + 1; j <= x1; ++j) {
                    field.put(new PointsImpl(j, y0), true);
                }
            }
        }
    ...
    private String getShotResponse(ClientHandler client, String turn) {
        ...
        PointsImpl point = new PointsImpl(Integer.parseInt(coordinates[0]), Integer.parseInt(coordinates[1]));

        Map<PointsImpl, Boolean> field;
        if (client.equals(playerOne)) {
        ...
        if (field.containsKey(point)) {
            ...
            List<PointsImpl> directions = new ArrayList<>(
                    Arrays.asList(
                            new PointsImpl(0, 1),
                            new PointsImpl(0, -1),
                            new PointsImpl(1, 0),
            ...
            for (int i = 0; i < directions.size(); ++i) {
                ...
                while (field.containsKey(new PointsImpl(nextX, nextY))) {
                    if (field.get(new PointsImpl(nextX, nextY))) {
                        return GOTHIM;
                    }
                    nextX += dx;
                    nextY += dy;
